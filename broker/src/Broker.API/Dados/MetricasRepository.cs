...
public class MetricasRepository : IDisposable
{
    private readonly InfluxDbSettings _settings;
    private readonly InfluxDBClient _influxDbClient;
    private readonly WriteApiAsync _writeApiAsync;

    public MetricasRepository(IConfiguration configuration)
    {
        _settings = ConfiguracoesUtils.ObterConfigInfluxDb(configuration);
        ArgumentNullException.ThrowIfNull(_settings);

        _influxDbClient = InfluxDbClientFactory.OpenConnection(_settings);
    ...
    public void LogRespostaProvedor(LogRespostaProvedorDto logRespostaProvedorDto)
    {
        try
        {
            ...
            _writeApiAsync.WritePointAsync(point, _settings.BucketPadrao, _settings.OrganizacaoPadrao);

            Log.Information(
                "Log de métricas da resposta do provedor registrado para a chamada no recurso {NomeRecurso}/{NomeProvedor}",
                logRespostaProvedorDto.NomeRecurso, logRespostaProvedorDto.NomeProvedor);
        }
        catch (Exception e)
        {
