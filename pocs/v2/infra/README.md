...
2. :warning: **Temporário:** gerar chave ssh para poder acessar a máquina com `ssh-keygen -t rsa -b 2048 -f ./keys/aws_key`. Esse nome já está configurado no Terraform para enviar a chave pública na instância. Mais a diante tem o comando para acessar a máquina via ssh com a chave privada
...
Para fazer a copia dos arquivos do bucket, utilizar `aws s3 sync s3://<BUCKET_ID>/broker src/`. O id do bucket também é exibido após aplicar a configuração.
